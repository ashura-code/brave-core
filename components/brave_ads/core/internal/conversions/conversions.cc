...
void Conversions::GetAdEventsCallback(
  ...
  ApplyFilters(redirect_chain, html, creative_set_conversions, ad_events);
}

void Conversions::ApplyFilters(
    const std::vector<GURL>& redirect_chain,
    const std::string& html,
    const CreativeSetConversionList& creative_set_conversions,
    const AdEventList& ad_events) {
  ...
  FilterAlreadyConvertedCreativeSets(filtered_creative_set_conversions,
                                     ad_events);

  FilterCreativeSetConversionsWithNonMatchingUrlPattern(
      filtered_creative_set_conversions, redirect_chain);

  BLOG(1, "There are " << filtered_creative_set_conversions.size() << " out of "
                       << creative_set_conversions.size()
                       << " eligible creative set conversions");

  CheckForConversions(redirect_chain, html, filtered_creative_set_conversions,
                      ad_events);
}
...
void Conversions::CheckForConversions(
  ...
  if (creative_set_conversion_buckets.empty()) {
    // There are no matching creative set conversion URL patterns for the
    // redirect chain.
    return;
  }
  ...
  for (const auto& ad_event : ad_events) {
    // Do we have a bucket containing creative set conversions for this ad
    // event?
    const auto iter =
    ...
    Convert(ad_event, MaybeBuildVerifiableConversion(
                          redirect_chain, resource_.get().id_patterns, html,
    ...
    if (creative_set_conversion_buckets.empty()) {
      // All the buckets are drained, so they will no longer contain
