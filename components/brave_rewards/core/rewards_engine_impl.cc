...
void RewardsEngineImpl::SetEnvironment(mojom::Environment environment) {
  DCHECK(IsUninitialized() || is_testing);
  _environment = environment;
}

void RewardsEngineImpl::SetDebug(bool debug) {
  DCHECK(IsUninitialized() || is_testing);
  is_debug = debug;
}

void RewardsEngineImpl::SetReconcileInterval(int32_t interval) {
  DCHECK(IsUninitialized() || is_testing);
  reconcile_interval = interval;
}

void RewardsEngineImpl::SetRetryInterval(int32_t interval) {
  DCHECK(IsUninitialized() || is_testing);
  retry_interval = interval;
}

void RewardsEngineImpl::SetTesting() {
  is_testing = true;
}

void RewardsEngineImpl::SetStateMigrationTargetVersionForTesting(
    int32_t version) {
...
void RewardsEngineImpl::GetDebug(GetDebugCallback callback) {
  std::move(callback).Run(is_debug);
}

void RewardsEngineImpl::GetReconcileInterval(
    GetReconcileIntervalCallback callback) {
  std::move(callback).Run(reconcile_interval);
}

void RewardsEngineImpl::GetRetryInterval(GetRetryIntervalCallback callback) {
