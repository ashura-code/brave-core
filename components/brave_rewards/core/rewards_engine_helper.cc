...
RewardsLogStream::RewardsLogStream(mojom::LedgerClient& client,
...
mojom::LedgerClient& RewardsEngineHelper::client() const {
  auto* client = context().GetLedgerImpl().client();
  CHECK(client);
  return *client;
}
