...
export const PortfolioNftAsset = () => {
  ...
  const onSend = React.useCallback(() => {
    ...
    const account = accounts
      .filter((account) => account.accountId.coin === selectedAssetFromParams.coin)
      .find((acc) => new Amount(getBalance(acc, selectedAssetFromParams)).gte('1'))

    if (!account) return
    
    console.log(WalletRoutes.SendPage.replace(':chainId?', selectedAssetsNetwork.chainId)
