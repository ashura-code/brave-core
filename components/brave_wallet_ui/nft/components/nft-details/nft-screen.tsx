...
// queries
import { useGetAutopinEnabledQuery, useGetNftMetadataQuery, useGetNftPinningStatusQuery } from '../../../common/slices/api.slice'
...
export const NftScreen = (props: Props) => {
  ...
  // queries
  const { data: nftMetadata, isLoading: isFetchingNFTMetadata, error: nftMetadataError } = useGetNftMetadataQuery(
    selectedAsset,
    {
      skip: !selectedAsset
    }
  ...
  const onMessageEventListener = React.useCallback((event: MessageEvent<CommandMessage>) => {
    ...
    if (event.origin === braveNftDisplayOrigin || event.origin === braveWalletPanelOrigin) {
