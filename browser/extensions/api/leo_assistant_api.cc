...
ExtensionFunction::ResponseAction LeoGetShowLeoAssistantIconFunction::Run() {
    auto* service = sidebar::SidebarServiceFactory::GetForProfile(Profile::FromBrowserContext(browser_context()));
    const auto hidden_items = service->GetHiddenDefaultSidebarItems();
    return RespondNow(WithArguments(!base::ranges::any_of(hidden_items, [](const auto& item) {
        return item.built_in_item_type ==
        sidebar::SidebarItem::BuiltInItemType::kChatUI;
    })));
}
...
ExtensionFunction::ResponseAction LeoResetFunction::Run() {
    ...
    profile->GetPrefs()->SetBoolean(ai_chat::prefs::kBraveChatHasSeenDisclaimer, false);
    profile->GetPrefs()->SetBoolean(ai_chat::prefs::kBraveChatAutoGenerateQuestions, false);
